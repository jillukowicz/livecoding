...
## our mixer !
master = range(1,100).mirror
bd_room = range(1, 1.5, 0.1).mirror

live_loop :drums do
  with_fx :lpf  do
    10.times do
      sample :bd_haus, amp: 1.5
      with_fx :gverb, room: bd_room.tick, mix: master.tick/100 do
        sample :bd_boom
      end
      sleep 0.5
    end
...
live_loop :wobble, sync: :drums do
  ...
  with_fx :echo do
    s = scale(:E2, :phrygian)
    ##| play s, release: 0.3, cutoff: master.tick, amp: 2
    sleep 1
